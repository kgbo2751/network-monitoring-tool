...
extern "C" __declspec(dllexport) const char* get_network_data_json_c() {
    ...
    jsonStr += "\"ipv4\":\"" + ipv4 + "\",";
    jsonStr += "\"subnet\":\"" + subnet + "\",";
    jsonStr += "\"gateway\":\"" + gateway + "\",";
    jsonStr += "\"mac\":\"" + mac + "\",";
    jsonStr += "\"dns\":\"" + dns + "\",";

    std::string downloadStr = "0 KB/s", uploadStr = "0 KB/s";
    PMIB_IFTABLE pIfTable = (MIB_IFTABLE*)malloc(sizeof(MIB_IFTABLE));
    ...
    if (pIfTable && GetIfTable(pIfTable, &dwSize, FALSE) == NO_ERROR) {
        ...
        for (DWORD i = 0; i < pIfTable->dwNumEntries; i++) {
            ...
            std::string name = (char*)row.bDescr;
            ...
            std::string status = (row.dwOperStatus == IF_OPER_STATUS_OPERATIONAL) ? "Operational" : 
                                 (row.dwOperStatus == IF_OPER_STATUS_DISCONNECTED) ? "Disconnected" : "Other";
            double speed = row.dwSpeed / 1000000.0;
            if (!firstIface) jsonStr += ",";
            jsonStr += "{\"name\":\"" + nameEscaped + "\",\"status\":\"" + status + "\",\"speed_mbps\":" + std::to_string(speed) + "}";
            firstIface = false;
        }
    ...
    jsonStr += "\"download\":\"" + downloadStr + "\",";
    jsonStr += "\"upload\":\"" + uploadStr + "\",";
    ...
    jsonStr += "\"ping\":\"" + pingStr + "\",";

    jsonStr += "\"inPackets\":" + std::to_string(inPackets) + ",";
    jsonStr += "\"outPackets\":" + std::to_string(outPackets) + ",";
    jsonStr += "\"inErrors\":" + std::to_string(inErrors) + ",";
    jsonStr += "\"outErrors\":" + std::to_string(outErrors) + ",";
    
    char lossBuf[32];
    sprintf(lossBuf, "%.2f", lossRate);
    jsonStr += "\"lossRate\":" + std::string(lossBuf) + ",";
    ...
    jsonStr += "\"tcpCount\":" + std::to_string(tcpCount) + ",";
    ...
    jsonStr += "\"udpCount\":" + std::to_string(udpCount) + ",";
    ...
    if (tableNew && GetExtendedTcpTable(tableNew, &extSize, TRUE, AF_INET, TCP_TABLE_OWNER_PID_ALL, 0) == NO_ERROR) {
        ...
        for (DWORD i = 0; i < tableNew->dwNumEntries; i++) {
            ...
            if (strcmp(procName, "Unknown") != 0) {
                ...
                std::string nameEscaped = "";
                for (char c : std::string(procName)) {
                    if (c == '\\') nameEscaped += "\\\\";
                    else if (c == '"') nameEscaped += "\\\"";
                    else nameEscaped += c;
                }
                
                if (!firstProc) jsonStr += ",";
                jsonStr += "{\"pid\":" + std::to_string(pid) + ",\"name\":\"" + nameEscaped + "\"}";
                firstProc = false;
            }
        }
    }
